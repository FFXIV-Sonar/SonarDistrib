...
public class WmiMacAddressDeviceIdComponent : IDeviceIdComponent
{
        ...
        foreach (var managementInstance in managementClass.GetInstances())
        {
            try
            {
                ...
                if (managementInstance["PermanentAddress"] is string permanentAddress)
                {
                    ...
                    values.Add(FormatMacAddress(permanentAddress));
                }
            }
            finally
            {
    ...
    public static string FormatMacAddress(string input) // straight out of decompilation MacAddressFormatter.FormatMacAddress
    {
        ...
        IEnumerable<string> source = from x in Enumerable.Range(0, input.Length / 2)
                                     select input.Substring(x * 2, 2);
